package com.mio.boot2.controller;
...
import com.mio.boot2.model.Customer;
import com.mio.boot2.model.CustomerRepository;
...
public class CustomerController {
	...
	public Flux<Customer> getCustomer() {
		...
		return Flux.fromIterable(customer);
	}
	...
	public Mono<ResponseEntity<String>> postCustomer(@RequestBody Customer customer) {
		...
		return Mono.just(
				new ResponseEntity<>("Post Successfully!", HttpStatus.CREATED));
	}

	@PutMapping("/put/{id}")
	public Mono<ResponseEntity<Customer>> putCustomer(@PathVariable Long id,
	...
	public Mono<ResponseEntity<String>> deleteMethod(@PathVariable Long id) {
		...
		return Mono.just(new ResponseEntity<>("Delete Succesfully!",
				HttpStatus.ACCEPTED));
	}
